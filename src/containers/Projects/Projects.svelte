<script lang="ts">
  import PortfolioItem from './components/PortfolioItem/PortfolioItem.svelte';
  import { PROJECTS } from './projects';

  const halfItems = Math.ceil(PROJECTS.length / 2);
  const colOne = PROJECTS.slice(0, halfItems);
